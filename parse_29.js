...
const fileText = fs.readFileSync('raw_prices_29.txt', 'utf8');
...
// Extrapolate flat from 29 to 31
for (const ref of Object.keys(grids)) {
  let extrapolated = grids[ref];
  
  if (extrapolated[29] === undefined) {
    extrapolated[29] = extrapolated[28];
  }
  
  for (let x = 30; x <= 31; x++) {
    extrapolated[x] = extrapolated[29];
  }
}
