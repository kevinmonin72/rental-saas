...
const fileText = fs.readFileSync('raw_prices_21_22.txt', 'utf8');
...
const chunks = fileText.split('logo-lokki-solutions');
...
// Ensure extrapolation from 22 to 31 is flat
for (const ref of Object.keys(grids)) {
  let extrapolated = grids[ref];
  
  // Fill missing day 22
  if (extrapolated[22] === undefined) {
    extrapolated[22] = extrapolated[21];
  }
  
  for (let d = 23; d <= 31; d++) {
    extrapolated[d] = extrapolated[22];
  }
}
