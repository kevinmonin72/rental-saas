...
async function main() {
  ...
  while (hasMore) {
    const { data, error } = await supabaseAdmin.from('equipment').select('id, reference').range(from, from + limit - 1);
    if (error) {
  ...
  const { data: bookingItems } = await supabaseAdmin.from('booking_items').select('equipment_id');
  const usedEqIds = new Set(bookingItems.map(bi => bi.equipment_id));
