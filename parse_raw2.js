...
const latestMsg = fs.readFileSync('raw_prices.txt', 'utf8');
...
// Ensure 0.5 days from HALF_DAY_PRICES
...
// Split the file by the day markers
const dayMarkers = [
...
// Re-split using custom boundaries to get 6 precise chunks
let splitContent = latestMsg;
let chunks = [];
for (let i = 5; i >= 0; i--) {
  let parts = splitContent.split(dayMarkers[i]);
  if (parts.length > 1) {
    chunks[i+1] = parts[1]; // day i+1
    splitContent = parts[0]; // remaining string for previous days
  } else {
    // If we are at day 1 and it didn't split perfectly, parts[0] is just the chunk.
    if (i === 0) {
      chunks[1] = parts[0];
    }
  }
...
for (const [ref, daysObj] of Object.entries(grids)) {
  ...
  // Safety check: ensure 1 to 6 are filled
  for(let d = 1; d <= 6; d++) {
