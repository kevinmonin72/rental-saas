...
let file1 = fs.readFileSync('raw_prices.txt', 'utf8');
const dayMarkers = [
  "pour 1 jour logo-lokki-solutions",
  "2jlogo-lokki-solutions",
  "3j\nlogo-lokki-solutions",
  "4j\nlogo-lokki-solutions",
  "5j\nlogo-lokki-solutions",
  "6j\nlogo-lokki-solutions"
];

let splitContent = file1;
let chunks1 = [];
for (let i = 5; i >= 0; i--) {
  let parts = splitContent.split(dayMarkers[i]);
  if (parts.length > 1) {
    chunks1[i+1] = parts[1];
    splitContent = parts[0]; 
  } else {
    if (i === 0) chunks1[1] = parts[0];
  }
}

for (let d = 1; d <= 6; d++) {
  if (!chunks1[d]) continue;
  const lines = chunks1[d].split('\n');
...
let file2 = fs.readFileSync('raw_prices_7_15.txt', 'utf8');
const chunks2 = file2.split(/logo-lokki-solutions/);

for (let idx = 1; idx < chunks2.length; idx++) {
  const d = idx + 6; 
  const lines = chunks2[idx].split('\n');
  let currentPrice = null;
  for (let i = 0; i < lines.length; i++) {
    let line = lines[i].trim();
    if (!line) continue;
...
for (const ref of Object.keys(grids)) {
  ...
  // Safety fill 1-15
  for(let d = 1; d <= 15; d++) {
    if(extrapolated[d] === undefined || extrapolated[d] === 0) { // FIXED to catch 0
       if (extrapolated[d-1] !== undefined && extrapolated[d-1] !== 0) {
         // Except for carte session which might legitimately be 0
         if (ref.includes('-CS') || ref.includes('INITIATION')) {
           extrapolated[d] = 0;
         } else {
...
console.log("Successfully parsed and injected PRICING_GRIDS up to 31 days with 0 fix.");
