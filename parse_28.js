...
const fileText = fs.readFileSync('raw_prices_28.txt', 'utf8');
...
// Extrapolate flat from 28 to 31
for (const ref of Object.keys(grids)) {
  let extrapolated = grids[ref];
  
  if (extrapolated[28] === undefined) {
    extrapolated[28] = extrapolated[27];
  }
  
  for (let x = 29; x <= 31; x++) {
    extrapolated[x] = extrapolated[28];
  }
}
