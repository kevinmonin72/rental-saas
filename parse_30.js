...
const fileText = fs.readFileSync('raw_prices_30.txt', 'utf8');
...
// Extrapolate flat from 30 to 31
for (const ref of Object.keys(grids)) {
  let extrapolated = grids[ref];
  
  if (extrapolated[30] === undefined) {
    extrapolated[30] = extrapolated[29];
  }
  
  extrapolated[31] = extrapolated[30];
}
