...
async function parsePrices() {
  ...
  // chunks[0] = "pour 1 jour " -> this is day 1
  // chunks[1] = "...\n2j" -> this means day 1 content and then "2j" at the end. Wait, no.
  // We can just use the literal text.
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Switch day tracking explicitly by looking for specific headers in the text.
    if (line.includes("pour 1 jour")) currentDays = 1;
    if (line === "2jlogo-lokki-solutions" || line.includes("2j") && lines[i+1] && lines[i+1].includes("logo-lokki-solutions")) currentDays = 2;
    if (line === "3jlogo-lokki-solutions" || line.includes("3j") && lines[i+1] && lines[i+1].includes("logo-lokki-solutions")) currentDays = 3;
    if (line === "4jlogo-lokki-solutions" || line.includes("4j") && lines[i+1] && lines[i+1].includes("logo-lokki-solutions") || line.includes("4j")) currentDays = 4;
    if (line === "5jlogo-lokki-solutions" || line.includes("5j") && lines[i+1] && lines[i+1].includes("logo-lokki-solutions") || line.includes("5j")) currentDays = 5;
    if (line === "6jlogo-lokki-solutions" || line.includes("6j") && lines[i+1] && lines[i+1].includes("logo-lokki-solutions") || line.includes("6j")) currentDays = 6;
